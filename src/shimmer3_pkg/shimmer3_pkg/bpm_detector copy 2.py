...
HRV_LIMIT = 0    # Maximum plausible change in heart rate between successive beats

class BPMDetectorNode(Node):
    def __init__(self):
        super().__init__('bpm_detector_node')
        # Parameters
        self.declare_parameter('ppg_buffer_length', 150)
        self.ppg_buffer_length = self.get_parameter('ppg_buffer_length').value 
        ...
        # Buffers
        self.ppg_buffer = collections.deque(maxlen=1000) # rolling buffer, deque is an optimized list with O(1) appending and popping
    ...
    def chunk_callback(self, msg):
        ...
        ppg_chunk = np.array(msg.data, dtype=float)
        self.ppg_buffer.extend(ppg_chunk)

        if len(self.ppg_buffer) >= self.ppg_buffer_length:
            window = np.array(list(self.ppg_buffer)[-self.ppg_buffer_length:], dtype=float)

        try:
            wd, m = hp.process(window, SAMPLERATE)
            bpm = m['bpm']  # HeartPy average BPM
            self.publisher_single.publish(Float32(data=bpm))
        ...
        for rd in rr_time:
            ...
            if len(bpm) > 1: # no-difference check for first element
                ...
                if abs(bpm_diff) > HRV_LIMIT: # limit exceeded (indicates unrealistic spike)
                    ...
                    bpm.pop()
                    bpm.append(bpm[-1] + self.sign(bpm_diff) * HRV_LIMIT) # limit the change to the limit value
            if bpm[-1] > UPPERLIMIT:
                bpm.pop()
                if len(bpm) == 0:
