# BlueZ rfcomm is deprecating, so this needs to be rewritten with PyBlueZ 
...
class ros2_shimmer3(Node):
    def __init__(self):
        ...
        self.declare_parameter('Channel_Number', "1")
        ...
        channel = int(self.Parm_Channel_Number)
        self.sock = None
        try:
            self.sock = bluetooth.BluetoothSocket(bluetooth.RFCOMM)
            self.sock.connect((bt_addr, channel))
            self.sock.settimeout(1.0)
            self.get_logger().info(f"Connected to {bt_addr} on channel {channel}")
